...
Checks:
1. Two metal clips are present (over wires only)
...
class StructuralInspector:
    """
    Args:
        blue_hue_low:        Lower HSV hue bound for blue hoop detection.
        blue_hue_high:       Upper HSV hue bound for blue hoop detection.
        min_hoop_area:       Minimum contour area to be considered a hoop.
        min_clip_area:       Minimum contour area for a clip.
        max_clip_area:       Maximum contour area for a clip.
        clip_min_brightness: Minimum brightness (HSV value) for clip detection.
    """
    ...
    def __init__(
        ...
        max_clip_area:       int = 5000,
        clip_min_brightness: int = 180,
    ):
    ...
    def inspect(
        ...
        """
        Run full structural inspection on image.
        Detection runs inside ROI only; results drawn on full image.

        Args:
            image:   (H, W, 3) uint8 RGB image.
            roi_cfg: ROI config dict from configs/config.yaml.

        Returns:
            InspectionResult with pass/fail and annotated image.
        """
        ...
        if hoop_found and clips_found >= 2:
            hoop_between = self._is_hoop_between_clips(hoop_contour, clips)
            if not hoop_between:
    ...
    def _detect_clips(
        ...
        """
        Detect metal clips using silver color masked to wire regions only.
        Clips are only accepted where colored wires are present underneath.
        """
        hsv = cv2.cvtColor(region, cv2.COLOR_RGB2HSV)

        # ── Build wire mask (orange, yellow, green wires) ──────────────
        ...
        wire_mask = cv2.bitwise_or(orange_mask, yellow_mask)
        wire_mask = cv2.bitwise_or(wire_mask,   green_mask)

        # Dilate wire mask — clips sit on top of wires
        kernel    = np.ones((15, 15), np.uint8)
        wire_mask = cv2.dilate(wire_mask, kernel, iterations=2)

        # ── Build silver/metallic mask ─────────────────────────────────
        ...
        # Remove blue hoop region from clip mask
        ...
        # ── Restrict clips to wire regions only ────────────────────────
        clip_mask = cv2.bitwise_and(silver_mask, wire_mask)
        ...
        contours, _ = cv2.findContours(
            clip_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        # Merge overlapping detections of the same clip
        clips = self._merge_overlapping_boxes(clips, overlap_thresh=0.3)

        # Sort left to right, return two largest
    ...
    def _is_hoop_between_clips(
        ...
        the right edge of the left clip and left edge of the right clip.
        """
        if len(clips) < 2:
            return False
        ...
        # Sort clips left to right by centre x
